...
class RadarObject():
    ...
    def getadcDataFromDCA1000(self, fileName):
        ...
        for i in range(0, fileSize, self.numADCSamples * 4):
            adcData[0][iter:iter + self.numADCSamples] = LVDS[0][i:i + self.numADCSamples] + np.sqrt(-1 + 0j) * LVDS[1][
                                                                                                                i:i + self.numADCSamples]
            adcData[1][iter:iter + self.numADCSamples] = LVDS[0][
                                                         i + self.numADCSamples:i + self.numADCSamples * 2] + np.sqrt(
                -1 + 0j) * LVDS[1][i + self.numADCSamples:i + self.numADCSamples * 2]
            adcData[2][iter:iter + self.numADCSamples] = LVDS[0][
                                                         i + self.numADCSamples * 2:i + self.numADCSamples * 3] + np.sqrt(
                -1 + 0j) * LVDS[1][i + self.numADCSamples * 2:i + self.numADCSamples * 3]
            adcData[3][iter:iter + self.numADCSamples] = LVDS[0][
                                                         i + self.numADCSamples * 3:i + self.numADCSamples * 4] + np.sqrt(
                -1 + 0j) * LVDS[1][i + self.numADCSamples * 3:i + self.numADCSamples * 4]
            iter = iter + self.numADCSamples

        # correct reshape 4, 115200, 256
    ...
    def generateHeatmap(self, frame):
        ...
        chirpPad = self.idxProcChirp // self.numGroupChirp # 64//4
        i = 0
    ...
    def processRadarDataHoriVert(self,bin_filename):
        all_heatmaps = []
        adcDataHori = self.getadcDataFromDCA1000(bin_filename)
        for idxFrame in range(0, self.numFrame):
            ...
            all_heatmaps.append(outputHori)
            print(bin_filename, idxFrame)
        return all_heatmaps
