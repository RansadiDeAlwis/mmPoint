...
try:
    ...
    print('⚠️ pointnet2_cuda not available — using fallback')
    from utils.pointnet2_fallback import pointnet2_utils as pn2
...
def position_encoding(p, downscale_p_by=0.5):
    n_freq_posenc = 10
    # Positional encoding
...
def get_repulsion_loss(pred, nsample=10, radius=0.07):
    radius = 0.15
    idx = knn(pred.transpose(1, 2).contiguous(), nsample).int()
    pred_flipped = pred.transpose(1, 2).contiguous()
    grouped_pred = pn2.grouping_operation(pred_flipped, idx) 
